...
export class SubjectService {
  ...
  public getAllSubjectsByUser(
    ...
    return this.httpClient.get<GetResponseSubject>(
      `${this.BASE_URL}/${userId}/all`,
      { params }
    );
  }

  public getSubjectDetails(subjectId: number): Observable<Subject> {
    return this.httpClient.get<Subject>(
      `${this.BASE_URL}/subject-details/${subjectId}`
    );
  }
  ...
  public getAllSubjectsForTeacher(
    ...
    const userId = Number(this.getAcctualUserId());
    return this.httpClient.get<GetResponseSubject>(
      `${this.BASE_URL}/${userId}/all-teacher`,
      { params }
    );
  }
  ...
  addMessage(subject: Subject): Observable<number> {
    ...
    return this.httpClient
      .post<number>(this.BASE_URL + `/new?teacherId=${userId}`, subject)
      .pipe();
  }

  updateSubject(subjectId: number, subject: Subject): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.httpClient.put(this.BASE_URL + `/${subjectId}`, subject).subscribe(
        (data) => {
  ...
  public getFiveFirstSubjectsByUser(week: string): Observable<Subject[]> {
    ...
    if (role == "ROLE_TEACHER") {
      return this.httpClient.get<Subject[]>(
        `${this.BASE_URL}/${userId}/five-first-teacher?week=${week}`
      );
    }
    if (role == "ROLE_STUDENT") {
      return this.httpClient.get<Subject[]>(
        `${this.BASE_URL}/${userId}/five-first?week=${week}`
      );
    }
  }
