...
import { GroupMessageFromSearch } from "../common/group-message-from-search";
...
export class MessageService {
  ...
  addGroupMessageFromSearch(message: GroupMessageFromSearch) {
    const userId = this.getAcctualUserId();
    message.userFrom = Number(userId);
    this.httpClient
      .post(`${this.BASE_URL}/group-message-search`, message)
      .subscribe();
  }
  ...
  getAllReadMessagesByUser(
    ...
    return this.httpClient.get<GetResponseMessage>(
      `${this.BASE_URL}/${userId}/all-read`,
      { params }
    );
  }
  ...
  getAllUnreadMessagesByUser(
    ...
    return this.httpClient.get<GetResponseMessage>(
      `${this.BASE_URL}/${userId}/all-unread`,
      { params }
    );
  }
  ...
  getAllSentMessagesByUser(
    ...
    return this.httpClient.get<GetResponseMessage>(
      `${this.BASE_URL}/${userId}/all-sent`,
      { params }
    );
  }

  getMessageDetails(messageId: number): Observable<MessageDetails> {
    return this.httpClient.get<MessageDetails>(
      `${this.BASE_URL}/message-details/${messageId}`
    );
  }

  changeMessageStatus(messageId: number) {
    this.httpClient
      .patch(`${this.BASE_URL}/update-status-message/${messageId}`, httpOptions)
      .subscribe();
  }
